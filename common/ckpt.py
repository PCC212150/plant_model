...
# 当前流程要求的输出通道数（见 config.CLASS_NAMES：shoot / above / sett）
# **别拿去加载 root_model 的权重**：那边也是 3 通道，但语义完全不同
# （root / stem / check），装进来不会报错、只会静默给出毫无意义的结果。
REQUIRED_CHANNELS = 3
...
def load_unet(pth, device="cpu", require: int = REQUIRED_CHANNELS):
    ...
    if require is not None and out_ch != require:
        raise SystemExit(
            ...
            f"{require} 通道（茎 shoot / 地上部 above / 种茎 sett）。"
            f"请用本项目 train.py 训练得到的权重。")
